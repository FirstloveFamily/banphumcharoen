@extends('layouts.staff-portal', ['title' => 'เพิ่มนายจ้างใหม่', 'pageTitle' => 'จัดการข้อมูลนายจ้าง'])
    ...
    <div class="space-y-8 max-w-5xl mx-auto">
        ...
        <header class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div class="flex items-center gap-5">
                ...
                <div>
                    <h2 class="text-3xl font-bold tracking-tight text-slate-900">เพิ่มนายจ้างใหม่</h2>
                    <p class="mt-1 text-slate-500">ลงทะเบียนข้อมูลบริษัทหรือสถานประกอบการใหม่เข้าสู่ระบบ</p>
                </div>
            </div>
        </header>
        ...
        <form action="{{ route('staff.portal.employers.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            ...
            <section class="glass-card rounded-3xl p-8 shadow-sm">
                ...
                <div class="grid md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        <label class="text-xs font-bold uppercase tracking-wider text-slate-500">รหัสบริษัท</label>
                        <input type="text" name="company_code" value="{{ old('company_code', $companyCode ?? '') }}" readonly
                            class="h-12 w-full rounded-2xl border border-slate-100 bg-slate-100 px-4 text-sm font-medium text-slate-600 outline-none">
                        <p class="text-xs text-slate-400">ระบบจะสร้างรหัสให้อัตโนมัติเมื่อบันทึกข้อมูล</p>
                    </div>
                    <div class="space-y-2">
                    ...
                    <div class="space-y-2">
                        <label class="text-xs font-bold uppercase tracking-wider text-slate-500">โลโก้บริษัท (.jpg, .png)</label>
                    ...
                    <div class="md:col-span-2 space-y-2">
                        ...
                        <textarea name="address" rows="3" placeholder="เลขที่อาคาร, ถนน, ตำบล, อำเภอ, จังหวัด..."
                            class="w-full rounded-2xl border border-slate-100 bg-slate-50/50 p-4 text-sm font-medium outline-none focus:border-blue-400 focus:bg-white transition-all">{{ old('address') }}</textarea>
                    </div>
                </div>
            </section>
            ...
            <section class="glass-card rounded-3xl p-8 shadow-sm">
                ...
                <div class="mt-8 space-y-2">
                    ...
                    <textarea name="notes" rows="3" placeholder="ระบุข้อมูลเพิ่มเติมถ้ามี..."
                        class="w-full rounded-2xl border border-slate-100 bg-slate-50/50 p-4 text-sm font-medium outline-none focus:border-blue-400 focus:bg-white transition-all">{{ old('notes') }}</textarea>
                </div>
