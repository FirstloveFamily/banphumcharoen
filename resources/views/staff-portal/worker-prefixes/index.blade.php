@extends('layouts.staff-portal', ['title' => 'คำนำหน้าชื่อ', 'pageTitle' => 'คำนำหน้าชื่อ'])
    ...
    <div class="space-y-8">
            ...
            <div class="flex flex-col gap-6 px-5 py-6 sm:px-7 lg:flex-row lg:items-end lg:justify-between lg:px-8">
                <div class="max-w-3xl">
                    ...
                    <p class="mt-2 max-w-2xl text-sm leading-7 text-slate-300 sm:text-base">
                        แยกข้อมูลคำนำหน้าชื่อเป็นตารางกลาง เพื่อใช้กับแรงงานทุกคนได้อย่างเป็นมาตรฐาน
                    </p>
                </div>

                <div class="flex flex-wrap items-center gap-3">
                    <a href="{{ route('staff.portal.worker-prefixes.create') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-[#0b2f52] via-[#123e68] to-[#b91c1c] px-4 py-3 text-sm font-extrabold text-white shadow-lg shadow-black/20 transition hover:opacity-95">
        ...
        <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <article class="manager-card manager-card-hover p-5">
                <p class="text-xs font-bold uppercase tracking-widest text-slate-500">ทั้งหมด</p>
            ...
            <article class="manager-card manager-card-hover p-5">
                <p class="text-xs font-bold uppercase tracking-widest text-slate-500">แรงงานที่ผูกใช้</p>
                <h3 class="mt-3 text-3xl font-extrabold text-[#0b2f52]">{{ number_format($summary['workers']) }}</h3>
            </article>
        </section>
        ...
        <section class="manager-card overflow-hidden">
            <div class="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                <div>
                    <h3 class="text-lg font-extrabold text-[#0b2f52]">รายการคำนำหน้าชื่อ</h3>
                    <p class="mt-1 text-sm text-slate-500">กดแก้ไขเพื่อปรับชื่อ รหัส และสถานะของคำนำหน้าชื่อ</p>
                </div>
                ...
                <table class="w-full text-left text-sm">
                    <thead>
                        <tr class="portal-table-head text-xs font-bold uppercase tracking-wider">
                            <th class="px-6 py-4">คำนำหน้าไทย</th>
                            <th class="px-4 py-4">คำนำหน้า EN</th>
                            <th class="px-4 py-4">รหัส</th>
                            <th class="px-4 py-4 text-center">แรงงานที่ใช้</th>
                            <th class="px-4 py-4 text-center">สถานะ</th>
                            <th class="px-6 py-4 text-right">การจัดการ</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                        @forelse($workerPrefixes as $workerPrefix)
                            <tr class="portal-row-hover transition-colors">
                                <td class="px-6 py-4">
                                    <p class="font-bold text-[#0b2f52]">{{ $workerPrefix->name_th }}</p>
                                </td>
                                <td class="px-4 py-4 font-semibold text-slate-700">{{ $workerPrefix->name_en }}</td>
                                <td class="px-4 py-4 font-mono text-xs font-bold text-slate-600">{{ $workerPrefix->code }}</td>
                                <td class="px-4 py-4 text-center font-bold text-slate-700">{{ number_format($workerPrefix->workers_count) }}</td>
                                <td class="px-4 py-4 text-center">
                                    <span @class([
                                        'inline-flex rounded-full px-3 py-1 text-[11px] font-bold ring-1 ring-inset',
                                        'bg-emerald-50 text-emerald-700 ring-emerald-600/20' => $workerPrefix->is_active,
                                        'bg-slate-100 text-slate-500 ring-slate-400/20' => ! $workerPrefix->is_active,
                                    ])>
                                        {{ $workerPrefix->is_active ? 'ใช้งานอยู่' : 'ไม่ใช้งาน' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('staff.portal.worker-prefixes.edit', $workerPrefix) }}"
                                        ...
                                        <form method="POST" action="{{ route('staff.portal.worker-prefixes.destroy', $workerPrefix) }}" onsubmit="return confirm('ต้องการลบคำนำหน้าชื่อนี้ใช่หรือไม่?')">
                                            @csrf
                                            @method('DELETE')
