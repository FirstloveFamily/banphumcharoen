@extends('layouts.staff-portal', ['title' => 'แก้ไขข้อมูลแรงงาน', 'pageTitle' => 'จัดการข้อมูลแรงงาน'])
    ...
    <div class="space-y-8 max-w-5xl mx-auto">
        ...
        <header class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div class="flex items-center gap-5">
                ...
                <div>
                    <h2 class="text-3xl font-bold tracking-tight text-slate-900">แก้ไขข้อมูลแรงงาน</h2>
                    <p class="mt-1 text-slate-500">ปรับปรุงข้อมูลและเอกสารสำคัญของแรงงานรายบุคคล</p>
                </div>
            </div>
        </header>
        ...
        <form action="{{ route('staff.portal.workers.update', $worker) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <section class="glass-card rounded-3xl p-8 shadow-sm">
                    ...
                    <div class="lg:col-span-1 space-y-2">
                        ...
                        <select name="employer_id" required class="h-12 w-full rounded-2xl border border-slate-100 bg-slate-50/50 px-4 text-sm font-medium outline-none focus:border-blue-400 focus:bg-white transition-all appearance-none">
                            ...
                            @foreach($employers as $employer)
                                <option value="{{ $employer->id }}" @selected(old('employer_id', $worker->employer_id) == $employer->id)>{{ $employer->company_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="space-y-2">
                        ...
                        <select name="nationality_id" required class="h-12 w-full rounded-2xl border border-slate-100 bg-slate-50/50 px-4 text-sm font-medium outline-none focus:border-blue-400 focus:bg-white transition-all appearance-none">
                            ...
                            @foreach($nationalities as $nationality)
                                <option value="{{ $nationality->id }}" @selected(old('nationality_id', $worker->nationality_id) == $nationality->id)>{{ $nationality->name_th }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="space-y-2">
                        ...
                        <select name="gender" class="h-12 w-full rounded-2xl border border-slate-100 bg-slate-50/50 px-4 text-sm font-medium outline-none focus:border-blue-400 focus:bg-white transition-all appearance-none">
                            ...
                            <option value="male" @selected(old('gender', $worker->gender) == 'male')>ชาย (Male)</option>
                            <option value="female" @selected(old('gender', $worker->gender) == 'female')>หญิง (Female)</option>
                        </select>
                    </div>
                    <div class="space-y-2">
                        ...
                        <input type="date" name="birth_date" value="{{ old('birth_date', $worker->birth_date?->format('Y-m-d')) }}" required
                    ...
                    <div class="lg:col-span-2 space-y-2">
                        ...
                        <div class="flex items-center gap-4">
                            @if($worker->photo_path)
                                <img src="{{ asset('storage/' . $worker->photo_path) }}" class="h-12 w-12 rounded-xl object-cover border border-slate-100 p-0.5">
                            @endif
                ...
                <div class="mt-8 grid md:grid-cols-3 gap-8 pt-8 border-t border-slate-50">
                    <div class="space-y-2">
                        <label class="text-xs font-bold uppercase tracking-wider text-slate-500">คำนำหน้า</label>
                        <select name="worker_prefix_id"
                            ...
                            @foreach($workerPrefixes as $workerPrefix)
                                <option value="{{ $workerPrefix->id }}" @selected(old('worker_prefix_id', $worker->worker_prefix_id) == $workerPrefix->id)>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="first_name_th" value="{{ old('first_name_th', $worker->first_name_th) }}" required
                            class="h-12 w-full rounded-2xl border border-slate-100 bg-slate-50/50 px-4 text-sm font-medium outline-none focus:border-blue-400 focus:bg-white transition-all">
                    </div>
                    <div class="space-y-2">
                        <label class="text-xs font-bold uppercase tracking-wider text-slate-500">นามสกุล (ไทย)</label>
                        <input type="text" name="last_name_th" value="{{ old('last_name_th', $worker->last_name_th) }}"
                            class="h-12 w-full rounded-2xl border border-slate-100 bg-slate-50/50 px-4 text-sm font-medium outline-none focus:border-blue-400 focus:bg-white transition-all">
                    </div>
                </div>
                ...
                <div class="mt-8 grid md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        <label class="text-xs font-bold uppercase tracking-wider text-slate-500">First Name (EN) <span class="text-rose-500">*</span></label>
                        <input type="text" name="first_name_en" value="{{ old('first_name_en', $worker->first_name_en) }}" required
                    ...
                    <div class="space-y-2">
                        <label class="text-xs font-bold uppercase tracking-wider text-slate-500">Last Name (EN)</label>
                        <input type="text" name="last_name_en" value="{{ old('last_name_en', $worker->last_name_en) }}"
                            class="h-12 w-full rounded-2xl border border-slate-100 bg-slate-50/50 px-4 text-sm font-medium outline-none focus:border-blue-400 focus:bg-white transition-all uppercase">
                    </div>
                </div>
                ...
                <div class="mt-8 pt-6 border-t border-slate-50">
                    <label class="group flex items-center gap-3 cursor-pointer">
                        ...
                        <div class="relative">
                            <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $worker->is_active))
                                class="peer sr-only">
                ...
                <div class="grid lg:grid-cols-2 gap-12">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid gap-6">
                            ...
                            <div class="space-y-2">
                                <label class="text-xs font-bold uppercase tracking-wider text-slate-500">อัปเดตไฟล์ Passport</label>
                                <div class="flex items-center gap-3">
                                    @if($worker->passport_file)
                                        <a href="{{ asset('storage/' . $worker->passport_file) }}" target="_blank" class="grid h-10 w-10 place-items-center rounded-xl bg-blue-50 text-blue-600 hover:bg-blue-100 shadow-sm border border-blue-100 transition-all">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid gap-6">
                            ...
                            <div class="space-y-2">
                                <label class="text-xs font-bold uppercase tracking-wider text-slate-500">อัปเดตไฟล์ Work Permit</label>
                                <div class="flex items-center gap-3">
                                    @if($worker->wp_file)
                                        <a href="{{ asset('storage/' . $worker->wp_file) }}" target="_blank" class="grid h-10 w-10 place-items-center rounded-xl bg-emerald-50 text-emerald-600 hover:bg-emerald-100 shadow-sm border border-emerald-100 transition-all">
                                            <i data-lucide="file-text" class="h-4 w-4"></i>
                                        </a>
                                    @endif
                ...
                <div class="grid lg:grid-cols-2 gap-12 mt-12 pt-12 border-t border-slate-50">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid gap-6">
                            ...
                            <div class="space-y-2">
                                <label class="text-xs font-bold uppercase tracking-wider text-slate-500">อัปเดตไฟล์ Visa</label>
                                <div class="flex items-center gap-3">
                                    @if($worker->visa_file)
                                        <a href="{{ asset('storage/' . $worker->visa_file) }}" target="_blank" class="grid h-10 w-10 place-items-center rounded-xl bg-amber-50 text-amber-600 hover:bg-amber-100 shadow-sm border border-amber-100 transition-all">
                                            <i data-lucide="file-text" class="h-4 w-4"></i>
                                        </a>
                                    @endif
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid gap-6">
                            ...
                            <div class="space-y-2">
                                <label class="text-xs font-bold uppercase tracking-wider text-slate-500">อัปเดตไฟล์รายงานตัว</label>
                                <div class="flex items-center gap-3">
                                    @if($worker->report_90_days_file)
                                        <a href="{{ asset('storage/' . $worker->report_90_days_file) }}" target="_blank" class="grid h-10 w-10 place-items-center rounded-xl bg-slate-100 text-slate-600 hover:bg-slate-200 shadow-sm border border-slate-200 transition-all">
                                            <i data-lucide="file-text" class="h-4 w-4"></i>
                                        </a>
                                    @endif
