    ...
    <div class="space-y-8">
        ...
        {{-- <section class="hero-section rounded-2xl p-8 lg:p-12">
            <div class="animate-fade-in">
                <div
                    class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/10 text-sm font-medium text-white mb-6">
                    <span class="w-2 h-2 rounded-full bg-amber-400 animate-pulse"></span>
                    Document Expiry Alerts
                </div>
                <h1 class="text-4xl lg:text-5xl font-bold text-white mb-4">
                    เอกสารใกล้หมดอายุ 1111
                </h1>
                ...
                <div class="grid grid-cols-3 gap-4 mt-8">
                    <div class="bg-white/10 backdrop-blur-sm rounded-xl p-5 border border-white/10">
                        ...
                        <p class="mt-2 text-3xl font-bold text-red-400">
                            {{ \App\Models\WorkerDocument::query()->whereNotNull('expiry_date')->whereDate('expiry_date', '<', now())->count() }}
                        </p>
                    </div>
                    <div class="bg-white/10 backdrop-blur-sm rounded-xl p-5 border border-white/10">
                        ...
                        <p class="mt-2 text-3xl font-bold text-amber-400">
                            {{ \App\Models\WorkerDocument::query()->whereNotNull('expiry_date')->whereBetween('expiry_date', [now(), now()->addDays(30)])->count() }}
                        </p>
                    </div>
                    <div class="bg-white/10 backdrop-blur-sm rounded-xl p-5 border border-white/10">
                        ...
                        <p class="mt-2 text-3xl font-bold text-blue-400">
                            {{ \App\Models\WorkerDocument::query()->whereNotNull('expiry_date')->whereBetween('expiry_date', [now(), now()->addDays(45)])->count() }}
                        </p>
                    </div>
                </div>
            </div>
        ...
        <section class="card rounded-2xl p-6 animate-fade-in">
            ...
            <div class="space-y-4">
                ...
                @forelse ($allExpiring as $document)
                    ...
                    <div
                        class="alert-card rounded-xl p-5 @if ($isCritical) alert-critical @elseif($isWarning) alert-warning @else alert-info @endif">
                        <div class="flex items-start justify-between gap-4">
                            <div class="flex-1">
                                <div class="flex items-center gap-3 mb-2">
                                    <span
                                        class="badge @if ($isExpired) badge-critical @elseif($isCritical) badge-critical @elseif($isWarning) badge-warning @else badge-info @endif rounded-lg px-3 py-1 text-sm font-semibold">
                                        @if ($isExpired)
                                            หมดอายุแล้ว
                                        @elseif($isCritical)
                                ...
                                <h4 class="text-lg font-semibold text-slate-900">
                                    {{ $document->worker?->full_name_th ?: $document->worker?->full_name_en }}</h4>
                                <p class="text-sm text-slate-600 mt-1">
                                    {{ $document->documentMaster?->name ?? 'เอกสาร' }}</p>
                                <p class="text-sm text-slate-500 mt-2">วันหมดอายุ:
                                    {{ $this->formatDate($document->expiry_date) }}</p>
                            </div>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-2 text-3xl font-bold text-slate-900">
                            {{ number_format(\App\Models\WorkerDocument::count()) }}</p>
                    </div>
                    <div class="w-12 h-12 rounded-xl bg-slate-100 flex items-center justify-center">
            ...
            <div class="card rounded-2xl p-6 animate-fade-in">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-2 text-3xl font-bold text-slate-900">
                            {{ number_format(\App\Models\Worker::count()) }}</p>
                    </div>
                    <div class="w-12 h-12 rounded-xl bg-slate-100 flex items-center justify-center">
            ...
            <div class="card rounded-2xl p-6 animate-fade-in">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-2 text-3xl font-bold text-slate-900">
                            {{ number_format(\App\Models\Employer::count()) }}</p>
                    </div>
                    <div class="w-12 h-12 rounded-xl bg-slate-100 flex items-center justify-center">
